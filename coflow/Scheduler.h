...
public:
	...
	void UpdateInfoFE(Flow *flow,Coflow *coflow,Flow f);//更新有错信息
	void UpdateCoflowFlowtable(Coflow *coflow);			//更新Coflow流表
	void UpdateCoflowFlowtableET(Coflow *coflow);		//容错机制更新Coflow流表
	...
	void EMPQ(Flow *flow, Coflow *coflow);				//错分类的多级优先级队列调度
	void ETMPQ(Flow *flow, Coflow *coflow);				//带有容错机制的多级优先级队列调度
